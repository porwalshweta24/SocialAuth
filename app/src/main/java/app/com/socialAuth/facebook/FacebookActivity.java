...
public class FacebookActivity extends Activity implements OnClickListener {
	...
	private Handler handler = new Handler(new Callback() {
		...
		public boolean handleMessage(Message msg) {
			...
			if (msg.what == 1) {
				Intent intent = new Intent(FacebookActivity.this, UserInfo.class);
				...
				if (friendList.size() > 0) {
					...
					Toast.makeText(FacebookActivity.this, "No friends found.",
				...
				if (session.isOpened()) {
					...
					for (int i = 0; i < session.getPermissions().size(); i++) {
						if (session.getPermissions().get(i).contains("email")) {
							pd = ProgressDialog.show(FacebookActivity.this, "", "");
	...
	private void getPermissionFromFacebook() {
		...
		Session.getActiveSession().requestNewReadPermissions(
				new NewPermissionsRequest(FacebookActivity.this, Arrays
						.asList(permissions)));
	}
